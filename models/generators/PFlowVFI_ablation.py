...
class SoftBinary(torch.nn.Module):
    def __init__(
        self, cin, dilate_size=7, mask_type="quasi-binary", noise=True
    ) -> None:
        super().__init__()
        channel = 64
        reduction = 8
        assert mask_type in ["quasi-binary", "binary", "adaptive"]
        self.dilation = not (mask_type == "binary")  # no dilation if binary
        self.adaptive = mask_type == "adaptive"  # use adaptive mask w/o sparsity
        ...
        if self.dilation:
            self.conv1 = torch.nn.Sequential(
                *[
                    torch.nn.Conv2d(
                        1, channel, dilate_size, 1, padding="same", bias=bias
                    ),
                    torch.nn.ReLU(inplace=True),
                    torch.nn.Conv2d(channel, channel, 3, 1, padding="same", bias=bias),
                    torch.nn.ReLU(inplace=True),
                    torch.nn.Conv2d(channel, channel, 1, 1, padding="same", bias=bias),
                ]
            )
            ...
            self.conv2 = torch.nn.Conv2d(channel, 1, 1, 1, padding="same", bias=bias)
